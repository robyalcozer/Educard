...
if (isset($_GET['quiz_number']) && $_GET['quiz_number'] >= 10 && $_GET['quiz_number'] <= 19) {
    $quiz_number = $_GET['quiz_number'];

    $stmt = $conn->prepare("SELECT question, choice1, choice2, choice3, choice4, correct_answer, quiz_title, timer FROM filipino_quiz WHERE quiz_number = ? ORDER BY RAND()");
...
$stmt = $conn->prepare("SELECT question, choice1, choice2, choice3, choice4, correct_answer, quiz_title, timer FROM filipino_quiz ORDER BY RAND()");
$stmt->execute();
$result = $stmt->get_result();
