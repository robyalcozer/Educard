...
if (isset($_GET['quiz_number']) && $_GET['quiz_number'] >= 30 && $_GET['quiz_number'] <= 39) {
    $quiz_number = $_GET['quiz_number'];

    $stmt = $conn->prepare("SELECT question, choice1, choice2, choice3, choice4, correct_answer, quiz_title, timer FROM math_quiz WHERE quiz_number = ? ORDER BY RAND()");
...
$stmt = $conn->prepare("SELECT question, choice1, choice2, choice3, choice4, correct_answer, quiz_title, timer FROM math_quiz ORDER BY RAND()");
$stmt->execute();
$result = $stmt->get_result();
