...
if (isset($_POST["question_count"])) {
    ...
    for ($i = 1; $i <= $_POST["question_count"]; $i++) {
        ...
        $insertquery = $conn->prepare("
            INSERT INTO english_quiz (correct_answer, question, choice1, choice2, choice3, choice4, quiz_number, quiz_title, timer)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
...
$sql_fetch_quiz = "SELECT quiz_number, subject, quiz_title FROM english_quiz GROUP BY quiz_number";
...
<head>
    ...
    <head>
        ...
        <title>Create: English Quiz</title>
...
<body>
    ...
    <div class="container">
        <form method="POST" id="quiz-form">
            ...
            <input type="number" id="subject" name="subject" placeholder="Please enter a quiz number between 20 and 29."
                required style="width: 68%; height: 45px;" min="20" max="29"> <br>
    ...
    <script>
        ...
        function validateQuizNumber() {
            ...
            var isValid = quizNumber >= 20 && quizNumber <= 29;

            if (!isValid) {
                alert("Please enter a quiz number between 20 and 29.");
            }

            document.getElementById("createBtn").disabled = !isValid;

            quizNumberInput.setCustomValidity(isValid ? "" : "Please enter a quiz number between 20 and 29.");
