...
if (isset($_GET['quiz_number']) && $_GET['quiz_number'] >= 1 && $_GET['quiz_number'] <= 9) {
    $quiz_number = $_GET['quiz_number'];

    $stmt = $conn->prepare("SELECT question, choice1, choice2, choice3, choice4, correct_answer, quiz_title, timer FROM science_quiz WHERE quiz_number = ? ORDER BY RAND()");
...
$stmt = $conn->prepare("SELECT question, choice1, choice2, choice3, choice4, correct_answer, quiz_title, timer FROM science_quiz ORDER BY RAND()");
$stmt->execute();
$result = $stmt->get_result();
