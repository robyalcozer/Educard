...
if (isset($_POST["question_count"])) {
    ...
    for ($i = 1; $i <= $_POST["question_count"]; $i++) {
        ...
        $insertquery = $conn->prepare("
            INSERT INTO science_quiz (correct_answer, question, choice1, choice2, choice3, choice4, quiz_number, quiz_title, timer)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
...
$sql_fetch_quiz = "SELECT quiz_number, subject, quiz_title FROM science_quiz GROUP BY quiz_number";
...
<head>
    ...
    <head>
        ...
        <title>Create: Science Quiz</title>
...
<body>
    ...
    <div class="container">
        <form method="POST" id="quiz-form">
            ...
            <input type="number" id="subject" name="subject" placeholder="Please enter a quiz number between 1 and 9."
                required style="width: 68%; height: 45px;" min="1" max="9"> <br>
    ...
    <script>
        ...
        function validateQuizNumber() {
            ...
            var isValid = quizNumber >= 1 && quizNumber <= 9;

            if (!isValid) {
                alert("Please enter a quiz number between 1 and 9.");
            }

            document.getElementById("createBtn").disabled = !isValid;

            quizNumberInput.setCustomValidity(isValid ? "" : "Please enter a quiz number between 1 and 9.");
